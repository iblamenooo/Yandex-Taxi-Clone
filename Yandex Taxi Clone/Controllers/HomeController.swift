...
class HomeController: UIViewController {
    ...
    private lazy var actionButton: UIButton = {
        ...
        button.setImage(UIImage(named: "baseline_menu_black_36dp")?.withRenderingMode(.alwaysOriginal),
                        for: .normal)
        ...
        PassengerService.shared.observeCurrentTrip { trip in
            ...
            case .denied:
                ...
                self.presentAlertController(withTitle: "Oops!",
                                            withMessage: "It looks like we couldn't find you a driver. Please try again.")
    ...
    func observeCancelledTrip(trip: Trip) {
            DriverService.shared.observeTripCancelled(trip: trip) {
                ...
                self.centerMapOnUserLocation()
                self.presentAlertController(withTitle: "Oops!",
                                            withMessage: "The passenger has decided to cancel this ride. Press OK to continue.")
            }
        }
    ...
    fileprivate func configureActionButton(config: ActionButtonConfig) {
        switch config {
        case .showMenu:
            self.actionButton.setImage(UIImage(named: "baseline_menu_black_36dp")?.withRenderingMode(.alwaysOriginal),
                            for: .normal)
            self.actionButtonConfig = .showMenu
        case .dismissActionView:
            actionButton.setImage(UIImage(named:                  "baseline_arrow_back_black_36dp")?.withRenderingMode(.alwaysOriginal),
                                  for: .normal)
...
extension HomeController: CLLocationManagerDelegate {
    func locationManager(_ manager: CLLocationManager, didStartMonitoringFor region: CLRegion) {
        ...
        if region.identifier == AnnotationType.destination.rawValue {
            print("DEBUG: Did start monitoring pick up destination region \(region)")
        }
...
extension HomeController: RideActionViewDelegate {
    func uploadTrip(_ view: RideActionView) {
        ...
        PassengerService.shared.uploadTrip(pickupCoordinates, destinationCoordinates) { err, ref in
            if let err = err {
                print("DEBUG: Failed to upload trip with \(err.localizedDescription)")
                return
            }
